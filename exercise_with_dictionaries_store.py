# -*- coding: utf-8 -*-
# Ni Xao это kaluginpeter, давай попробуем поиграться со словарями и прочей информацией
...
# Рассчитаем на какую сумму лежит каждого товара на складе.
#
...
#
# Алгоритм должен получиться(по крайней мере именно так я его себе представляю) приблизительно такой:
#
...
for good, id in goods.items():
    quantity, price = 0, 0
    for variant in store[id]:
        quantity += variant['quantity']
        price += variant['quantity'] * variant['price']
    print(good, quantity, price)
